...
def _save_live_tables(tables: Dict[str, pd.DataFrame]) -> None:
    live_dir = get_project_root() / "data" / "live"
...
def _build_proxy_pnr_from_synergy(synergy: pd.DataFrame, sample_pnr: pd.DataFrame) -> pd.DataFrame:
    ...
    top = top.dropna(subset=["ppp_value"]).sort_values("ppp_value", ascending=False).head(8)
        ...
        rows.append(
            {
                ...
                "shared_minutes": 0,
                "net_rating": 0,
                "coverage_note": "Live Synergy play-type row; exact handler-screener duo unresolved without possession/pass enrichment",
            }
        )
...
def build_dataset(prefer_live: bool = False, season: str = SEASON, season_type: str = SEASON_TYPE) -> Tuple[Dict[str, pd.DataFrame], pd.DataFrame]:
    ...
    if not prefer_live:
        manifest = pd.DataFrame(
            [
                {
                    "Table": key,
                    "Source Used": "Cleaned sample CSV",
                    "Status": "Manual sample mode",
                    "Caveat": "You selected cleaned sample data. Switch to live-first mode for ingestion attempts.",
                }
    ...
    if team_stats.empty:
        ...
        manifest_rows.append({"Table": "team_stats", "Source Used": "Cleaned sample fallback", "Status": "Attempted live, fell back", "Caveat": "NBA.com/stats team request failed or returned empty."})
    else:
        ...
        manifest_rows.append({"Table": "team_stats", "Source Used": "NBA.com/stats via nba_api", "Status": "Live", "Caveat": "Paint/perimeter defense may remain proxy without opponent tracking splits."})
    ...
    if player_stats.empty:
        ...
        manifest_rows.append({"Table": "player_stats", "Source Used": "Cleaned sample fallback", "Status": "Attempted live, fell back", "Caveat": "NBA.com/stats player request failed or returned empty."})
    else:
        player_stats = _merge_bref(player_stats, bref)
        for col in sample["player_stats"].columns:
            if col not in player_stats.columns:
                if col in ["player", "team", "position", "status", "role"]:
                    player_stats[col] = ""
                else:
                    player_stats[col] = 0
    ...
    if shot_zones.empty:
        ...
        manifest_rows.append({"Table": "shot_zones", "Source Used": "Cleaned sample fallback", "Status": "Attempted live, fell back", "Caveat": "ShotChartDetail request failed, timed out, or player IDs were unavailable."})
    else:
    ...
    if lineups.empty:
        ...
        manifest_rows.append({"Table": "lineups", "Source Used": "Cleaned sample fallback", "Status": "Attempted live, fell back", "Caveat": "NBA.com/stats lineup request failed or returned empty."})
    else:
    ...
    pnr_source = "NBA SynergyPlayTypes via nba_api" if not synergy.empty else "Cleaned sample fallback"
    pnr_caveat = "Exact handler-screener duo and coverage labels require deeper possession/pass tracking." if not synergy.empty else "Synergy request failed or returned empty."
    ...
    manifest_rows.append({"Table": "matchups", "Source Used": "Cleaned sample fallback", "Status": "Sample/proxy only", "Caveat": "MatchupsRollup/BoxScoreMatchupsV3 can be added in a later pass."})
